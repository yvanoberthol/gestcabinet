...
public interface RvRepository extends CrudRepository<Rv, Long> {
	...
	@Query("select rv from Rv rv where rv.client in (select c from Client c where c.id = ?1) and rv.jour > NOW() order by rv.jour,rv.creneau.hdebut,rv.creneau.mdebut")
